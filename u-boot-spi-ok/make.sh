...
pack_idb_imgage()
{
	...
	echo "./tools/mkimage -n ${PLAT} -T rksd -d ${TPL_BIN}:${SPL_BIN} idbloader.img"
...
function pack_uboot_image()
{
	...
	echo "${SCRIPT_UBOOT} --load ${LOAD_ADDR} ${PLAT_UBOOT_SIZE}"
...
function pack_loader_image()
{
	rm *_loader_*.bin -f
	cd ${RKBIN}
	echo "${SCRIPT_LOADER} --ini ${INI_LOADER}"
...
function pack_trust_image()
{
	...
	cd ${RKBIN}
	if [ "${ARM64_TRUSTZONE}" == "y" ]; then
		echo "${SCRIPT_ATF} --ini ${INI_TRUST} ${PLAT_SHA} ${PLAT_RSA} ${PLAT_TRUST_SIZE}"
		${SCRIPT_ATF} --ini ${INI_TRUST} ${PLAT_SHA} ${PLAT_RSA} ${PLAT_TRUST_SIZE}
	else
		echo "${SCRIPT_TOS} --ini ${INI_TRUST} --base ${DRAM_BASE} ${PLAT_TRUST_SIZE}"
...
function pack_spi_images() {
	  ...
	  LOAD_ADDR=`sed -n "/CONFIG_SYS_TEXT_BASE=/s/CONFIG_SYS_TEXT_BASE=//p" include/autoconf.mk|tr -d '\r'`
	  echo "./tools/loaderimage --pack --uboot u-boot.bin uboot_small.img ${LOAD_ADDR} --size 1024 1"
    ...
    cd ${RKBIN}
    if [ "${ARM64_TRUSTZONE}" == "y" ]; then
	  echo "${SCRIPT_ATF} --ini ${ini} ${PLAT_SHA} ${PLAT_RSA} --size 1024 1"
      ${SCRIPT_ATF} --ini ${ini} ${PLAT_SHA} ${PLAT_RSA} --size 1024 1
    else
	  echo "${SCRIPT_TOS} --ini ${ini} --base ${DRAM_BASE} --size 1024 1"
...
PartSize=0x800
EOF
    echo "./tools/firmwareMerger -P spi.ini ."
...
{
	echo "handle_args_late() ${ARG_LIST_FIT} --ini-trust ${INI_TRUST} --ini-loader ${INI_LOADER}"
...
function pack_images()
{
	...
	echo "pack_images() PLAT_TYPE=${PLAT_TYPE}"
